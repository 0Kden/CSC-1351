...
public class Movie implements Comparable<Movie> {
	...
	public int compareTo(Movie other) {
		...
		if(movieTitle.compareTo(((Movie) other).getTitle())>0) {
			return 1;
		}
		else if(movieTitle.compareTo(((Movie) other).getTitle())<0) {
			return -1;
		}
		else {
			if(releaseYear<((Movie) other).getYear())
				return-1;
			else if(releaseYear == ((Movie) other).getYear())
				return 0;
			else
				return 1;
			}
