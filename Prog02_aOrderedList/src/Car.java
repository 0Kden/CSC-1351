...
public class Car implements Comparable<Car> {
	...
	public int compareTo(Comparable oList) {	//compares two objects
		
		if(make.compareTo(((Car) oList).getMake())>0) {
			return 1;
		}
		else if(make.compareTo(((Car) oList).getMake())<0) {
			return -1;
		}
		else {
			if(year<((Car) oList).getYear())
				return-1;
			else if(year == ((Car) oList).getYear())
				return 0;
			else
				return 1;
		}
